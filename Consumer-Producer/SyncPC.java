      ...
      public void run() {
        while (z == 1) {
          ...
          try {
            ...
            try {
              m.runOnUiThread(new Runnable() {
                // The current thread is a producer that produces a value s, updates some UI
                // components, adds a record to an array, and inserts data into a database.
                @Override
                public void run() {
        ...
        // Clear the table and reset the producer and consumer labels after the
      ...
      public void run() {
        while (z == 1) {
          ...
          if (s == -1) {
            try {
              m.runOnUiThread(new Runnable() {
                // Maintain the state of the simulation and track the history of operations
